"""
Tests for app/ibkr_client.py's live-price fallback logic — kept separate
...
from app.ibkr_client import _MOCK_QUOTES, _live_or_fallback_price, ibkr_client


def test_live_or_fallback_price_uses_live_price_when_available():
    with patch("app.ibkr_client.fetch_current_price", return_value=205.50):
...
def test_live_or_fallback_price_preserves_a_legitimate_zero_price():
    ...
    with patch("app.ibkr_client.fetch_current_price", return_value=0.0):
...
def test_get_positions_assigns_each_symbol_its_own_price():
    ...
    with patch("app.ibkr_client.fetch_current_price", side_effect=lambda s: prices[s]):
